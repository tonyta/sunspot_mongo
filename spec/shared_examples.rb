shared_examples 'a mongo document' do
  ...
  describe '#reindex' do
    it 'should be able to reindex' do
      expect(subject.solr_search {}.hits.length).to eq 0
      subject.create(title: 'Test 1')
      subject.create(title: 'Test 2')
      subject.reindex
      expect(subject.solr_search {}.hits.length).not_to be_zero
      expect(subject.solr_search {}.hits.length).to eq subject.count
    end
    ...
    it 'should be able to reindex the result of an ORM query', if: ENV['MONGOID_VERSION'] do
      ...
      expect(subject.solr_search {}.hits.length).to eq 2
    end
  end
end
